...
- **`references/graphify-guide.md`** — **required** [Graphify](https://github.com/Graphify-Labs/graphify): query `graphify-out/` before grepping; skill **`graphify`** for ongoing use; `scripts/setup-graphify.sh` is **initial setup only** (`--rebuild` = start from scratch, not incremental)
